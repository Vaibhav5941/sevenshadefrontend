...
export default function ProductDetailComponent(props) {
    ...
    const showMainProduct = () => {
        return items.map((item) => {
            return <div style={{ display: 'flex', flexDirection:md_matches?'column': 'row', justifyContent: 'center', margin:md_matches?'0%': '1%', }}>
                <div>
                    <img src={`${serverURL}/static/${item}`} loading="lazy" style={{ width:md_matches?"100%":"100%", height: "100%", marginRight:md_matches?0: 20 }} />
                </div>
            </div>
        })
    ...
    const showAllDetails = () => {
        ...
        return <div style={{ display: 'flex', flexDirection: 'column', margin: '2px', flexWrap: 'wrap', maxWidth: '350px' }}>
            ...
            <div style={{ color: '#2d2d2d', fontSize:md_matches?'3vw': '1vw', fontWeight: 600, letterSpacing: 0.6 }}>{product?.productsubname}</div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap' }}>
                ...
                <div style={{ marginLeft: 5, marginTop: 12, fontSize: 16, letterSpacing: 0.4 }}>{product?.color}</div>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap' }}>
            ...
            <div style={{ fontSize: '20', marginTop: 10, width: 250 }}>
                <select style={{ width: '200px', fontSize:md_matches?'12px': '16px', border: '1px solid #ccc', borderRadius: '3px', padding: '5px 15px' }}>
                        ...
                        <div style={{ marginTop: 5, letterSpacing: 0.6, marginLeft: 30, textAlign: 'right' }}>
                            <ul style={{ listStyleType: 'none', padding: 0 }}>
                                <div style={{ display: 'flex', flexWrap: 'wrap', margin: '2px' }}>
                                    ...
                                    <img src={`${serverURL}/static/squareicon.png`} loading="lazy" style={{ width: 15, height: 15, marginTop: 2,marginLeft:5 }} />
                                </div>
                            </ul>
                        </div>
            ...
            <div style={{ marginTop: 0, fontSize: 10, letterSpacing: 0.6, marginLeft: 5, textAlign: 'left' }}>
                <ul style={{ listStyleType: 'none', padding: 0 }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', margin: '2px' }}>
                        ...
                        <img src={`${serverURL}/static/squareicon.png`} loading="lazy" style={{ width: 15, height: 15, marginTop: 3, marginLeft: 2 }} />
                    </div>
                </ul>
    ...
    return (<div style={{ width: '100%', justifyContent: 'center', marginLeft:md_matches?20: -300, maxWidth:md_matches?"100%": 1200 }}>
        <Grid container spacing={md_matches?0.1:0.1} style={{ width: '100%' }}>
            <Grid item xs={md_matches?12:12}>

                <div style={{ display: 'flex', flexWrap: 'wrap', margin: '2px', justifyContent: 'center',display:'flex',flexDirection:md_matches?'column':'row' }}>
                {md_matches?<div></div>:
                    <Grid item xs={2}>
                    ...
                    <Grid item xs={md_matches?12:6}>
                        <div style={{ width:md_matches?'100%': '90%', position: 'relative', maxwidth:md_matches?'100%': 100 }}>
                            <div style={{ cursor: 'pointer', position: 'absolute', left:md_matches?"10%": '2%', top:md_matches?'45%': '45%', zIndex: 3 }} onClick={handlePrevious2}><ArrowBackIosIcon style={{ color: 'grey', fontSize:md_matches?'5vw': '2vw' }} /></div>
                            <Slider ref={sldr2} {...settings}>
                                {showMainProduct()}
                            </Slider>
                            <div style={{ cursor: 'pointer', position: 'absolute', right:md_matches?"10%": '2%', top: '45%', zIndex: 3 }} onClick={handleNext2}><ArrowForwardIosIcon style={{ color: 'grey', fontSize:md_matches?'5vw': '2vw' }} /></div>
                        </div>
                    </Grid>
                    <Grid item xs={md_matches?12:4}>
